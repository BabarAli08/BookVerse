...
const MyLibrary = () => {
  ...
  const {
    profile,
    currentlyReading: supabaseCurrentlyReading,
    completedBooks: supabaseCompletedReading,
  } = useSelector((state: RootState) => state.userSettings);

  useEffect(() => {
    setUser(profile);

    const currentlyReading = supabaseCurrentlyReading.map((book) => {
      return {
        id: book.bookId,
        user_id: user?.id,
        title: book.title,
    ...
    const completedReading = supabaseCompletedReading.map((book) => {
      return {
        id: book.bookId,
        user_id: user?.id,
        title: book.title,
    ...
    setCurrentlyReading(currentlyReading as book[]);
    setCompletedReading(completedReading as book[]);
  }, [dispatch]);
  ...
  const handleNavigate = (status: string, book: book) => {
    ...
    if (status !== "free" && currentPlan.name !== "free") {
      navigate(`/books/${book?.id}`);
    }
    if (status !== "free" && currentPlan.name == "free") {
