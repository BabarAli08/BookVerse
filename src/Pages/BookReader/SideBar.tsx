import { Check, BookOpen, Bookmark, ChevronDown, PenTool, Settings, Moon, Sun } from 'lucide-react';
...
const ReadingSidebar = () => {
  ...
  const { fontFamily, highlited: highlights, notes, fontSize, lineHeight, togglDark, letterSpacing } = useSelector((state: RootState) => state.bookReading);
    ...
    <div className={`w-80 h-screen border-r ${
      ...
      <div className="h-full overflow-y-auto custom-scrollbar">
        ...
        <div className="p-4 border-b border-gray-200 dark:border-gray-700">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-3">
              <BookOpen className={`w-5 h-5 ${togglDark ? 'text-blue-400' : 'text-blue-600'}`} />
              <div>
        ...
        <div className="p-4">
          ...
          >
            <div className="flex items-center gap-3">
              <Settings className={`w-4 h-4 ${togglDark ? 'text-gray-400' : 'text-gray-600'}`} />
        ...
        <div className="p-4 border-t border-gray-200 dark:border-gray-700">
          ...
          >
            <div className="flex items-center gap-3">
              <PenTool className={`w-4 h-4 ${togglDark ? 'text-gray-400' : 'text-gray-600'}`} />
              <h2 className={`font-medium ${
                togglDark ? 'text-gray-100' : 'text-gray-900'
              }`}>Highlights</h2>
            </div>
          ...
          {expandedSections.highlights && (
            <div className="mt-3 space-y-2 px-3">
              {highlights.map((highlight, index) => (
                <div
                  key={index}
                  className={`p-3 rounded-lg ${getHighlightColor(highlight.color)}`}
                  style={{ 
                    backgroundColor: highlight.color ? `${highlight.color}30` : '#fbbf2430'
                  }}
                >
              ...
              {notes.length === 0 ? (
                ...
                notes.map((note, index) => (
                  ...
                  >
                    ...
                    <blockquote className={`text-xs italic mb-2 p-2 rounded ${
                      togglDark ? 'bg-gray-700/50 text-gray-300' : 'bg-gray-100 text-gray-600'
                    }`}>
                      "{note.selectedText}"
                    </blockquote>
                    <p className={`text-sm ${
        ...
        <div className="p-4 border-t border-gray-200 dark:border-gray-700">
          ...
          >
            <div className="flex items-center gap-3">
              <Bookmark className={`w-4 h-4 ${togglDark ? 'text-gray-400' : 'text-gray-600'}`} />
              <h2 className={`font-medium ${
                togglDark ? 'text-gray-100' : 'text-gray-900'
              }`}>Bookmarks</h2>
            </div>
              ...
              {bookmarks.map((bookmark) => (
                ...
                >
                  ...
                  <span className={`text-sm flex-1 ${
                    bookmark.completed 
                      ? togglDark ? 'line-through text-gray-500' : 'line-through text-gray-400'
                      : togglDark ? 'text-gray-200' : 'text-gray-700'
                  }`}>
                    {bookmark.title}
                  </span>
